...
export default function ExpensesPage() {
  ...
  const categories = ["Todas Categorias", ...new Set(expenses.map(e => e.category))];

  // Filter expenses by search query and category
  const filteredExpenses = expenses.filter(
    (expense) => {
      const matchesSearch = expense.description.toLowerCase().includes(searchQuery.toLowerCase());
      const matchesCategory = categoryFilter === "Todas Categorias" || expense.category === categoryFilter;
  ...
  return (
    <div>
      <div className="flex justify-between items-center mb-6">
        ...
        <Button className="anibtn-drawstroke" onClick={handleAddExpense}>+ Nova Despesa</Button>
      </div>

      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <div className="bg-white rounded-lg shadow p-6">
          <div className="flex items-center mb-4">
            <h2 className="text-xl font-semibold flex-grow">Gerenciamento de Despesas</h2>
          </div>
              ...
              <tbody>
                {filteredExpenses.map((expense) => (
                  <tr key={expense.id} className="border-b hover:bg-gray-50">
                    ...
                    <td className="p-3">{new Date(expense.dueDate).toLocaleDateString('pt-BR')}</td>
                    <td className="p-3">
          ...
          <div className="flex justify-between items-center mt-4">
            <Button variant="outline" size="sm" onClick={() => setCategoryFilter("Todas Categorias")}>
              Limpar filtros
            </Button>
            <div className="text-sm text-gray-500">
                          Total: {filteredExpenses.length} despesas [ R$ {totalValue.toFixed(2)} ]
            </div>
          </div>
        </div>
        ...
        <div className="bg-white rounded-lg shadow p-6">
          <div className="flex items-center mb-4">
            ...
            <h2 className="text-xl font-semibold">Serviços com Pagamento em Atraso</h2>
          </div>
                ...
                {overdueServices.map((service) => {
                  ...
                  return (
                    <tr key={service.id} className="border-b hover:bg-gray-50">
                      <td className="p-3">{client?.name || 'Cliente não encontrado'}</td>
                      <td className="p-3">{service.name}</td>
                      <td className="p-3">{new Date(service.date).toLocaleDateString('pt-BR')}</td>
